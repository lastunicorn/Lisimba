...
namespace DustInTheWind.Lisimba.Egg
{
    public class AddressBookManager
    {
        ...
        //    catch (Exception ex)
        //    {
        //        throw new EggException("Error loading from \"" + fileName + "\" file. Inner error message: " + ex.Message, ex);
        //    }
        //    finally
        //    {
        //        if (xr != null)
        ...
        public AddressBook LoadFromFile(string fileName)
        {
            ...
            using (ZipInputStream zs = new ZipInputStream(File.OpenRead(fileName)))
            {
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    if (lsbVersion == null)
                    {
                        OnIncorrectXmlVersion(new IncorrectXmlVersionEventArgs(eggVersion, lsbVersion, fileName));
                    }
                    else
                    {
                        int verCmp = 0;
        ...
        public bool SaveToFile(AddressBook addressBook)
        {
        ...
        public bool SaveToFile(AddressBook addressBook, string fileName)
        {
            bool returnValue = false;
        ...
        public bool ExportToFile(AddressBook addressBook, string fileName, FileFormat fileFormat)
        {
            bool returnValue = false;
